...
class DatabaseManager:
    ...
    def registrar_ponto(self, cpf: str, tipo_registro: str, foto_path: Optional[str] = None, 
                       lat: Optional[float] = None, lng: Optional[float] = None, 
                       obs: Optional[str] = None) -> tuple[bool, str]:
        """
        ...
        try:
            # Obter funcionário
            funcionario = self.obter_funcionario_por_cpf(cpf)
            if not funcionario:
            ...
            registro = RegistroPonto(
                ...
                ts_device=now.strftime("%Y-%m-%d %H:%M:%S"),
            ...
            registro_existente = self.obter_registro_do_dia(cpf, data_str)
            if registro_existente:
                ...
                # Preservar outros dados se não fornecidos
                if not registro.gps_lat: registro.gps_lat = registro_existente.get("gps_lat")
                if not registro.gps_lng: registro.gps_lng = registro_existente.get("gps_lng")
                if not registro.foto_path: registro.foto_path = registro_existente.get("foto_path")

            # Salvar
            self.salvar_registro(registro)
            
            return True, f"{tipo_registro.replace('_', ' ').title()} registrado com sucesso!"
        ...
        with self._connect() as conn:
            ...
            if original:
                ...
                self.registrar_auditoria(
                    ...
                    "SYSTEM", 
                    "DELETE_REGISTRO", 
                    f"Exclusão de ponto ID {registro_id}. Dados: {dados_antigos}"
                )
    ...
    def obter_registro_por_id(self, registro_id: int) -> Optional[Dict[str, Any]]:
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM registros WHERE id=?", (registro_id,))
            row = cursor.fetchone()
    ...
    def salvar_departamento(self, nome: str) -> int:
        with self._connect() as conn:
            cursor = conn.cursor()
            try:
                cursor.execute("INSERT INTO departamentos (nome) VALUES (?)", (nome,))
                ...
                # Se já existe, retorna o ID do existente
                cursor.execute("SELECT id FROM departamentos WHERE nome=?", (nome,))
                row = cursor.fetchone()
    ...
    def get_config(self, key: str, default: str = "") -> str:
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT v FROM app_config WHERE k=?", (key,))
            row = cursor.fetchone()
    ...
    def set_config(self, key: str, value: str):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("REPLACE INTO app_config (k, v) VALUES (?, ?)", (key, value))
            conn.commit()
