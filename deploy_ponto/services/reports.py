...
class ReportService:
    ...
    def _obter_periodo_fechamento(self, ano: int, mes: int):
        # Ler configuração do banco de dados (prioridade) ou usar padrão 1
        dia_fechamento_str = self.db.get_config("dia_fechamento", "1")
        tipo_fechamento = self.db.get_config("tipo_fechamento", "MES_ANTERIOR")
        ...
        if dia_fechamento == 1:
            ...
            if tipo_fechamento == "PROXIMO_MES":
                # Ex: Ref Maio (05), Fechamento 21
                # Inicio: 21/05
                # Fim: 20/06
                
                try:
                    data_inicio = date(ano, mes, dia_fechamento)
                except ValueError:
                    ultimo_dia = calendar.monthrange(ano, mes)[1]
                    data_inicio = date(ano, mes, min(dia_fechamento, ultimo_dia))
                ...
                if dia_fim_periodo < 1: dia_fim_periodo = 1
                
                try:
                    data_fim = date(ano_fim, mes_fim, dia_fim_periodo)
                except ValueError:
                    ultimo_dia_fim = calendar.monthrange(ano_fim, mes_fim)[1]
                    data_fim = date(ano_fim, mes_fim, min(dia_fim_periodo, ultimo_dia_fim))
    ...
    def gerar_espelho_ponto_html(self, funcionario_id: int, ano: int, mes: int) -> str:
        ...
        # Dados da Empresa
        empresa_razao = self.db.get_config("empresa_nome", "EMPRESA NÃO CONFIGURADA")
        empresa_cnpj = ""
        empresa_endereco = self.db.get_config("empresa_endereco", "")
